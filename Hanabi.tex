...
\textit{Hanabi} is a cooperative card game for 2-5 players. A Hanabi deck is composed of 50 cards, 10 for each of the colours \textcolor{red}{red} \R{S}, \textcolor{yellow}{yellow} \Y{S}, \textcolor{green}{green} \G{S}, \textcolor{blue}{blue} \B{S}, and \textcolor{gray}{white} \W{S}. We use black to denote a generic colour.

The values of the 10 cards of each colour are \C{K1} \C{K1} \C{K1} \C{K2} \C{K2} \C{K3} \C{K3} \C{K4} \C{K4} \C{K5}.

There are two variants that include an optional sixth colour \textcolor{violet}{rainbow} \M{S}. In the \textit{rainbow variant}, the rainbow cards work exactly as the other ones, except that there's exactly one copy of each card of value from 1 to 5. In the \textit{multicolour variant}, the rainbow cards distribution is the same for the other colours, but you can't give a \textit{rainbow} clue, and you should point all the rainbow cards when giving any colour clue.
...
\begin{tasks}(2)
	\task[$\bullet$] No clues. \C{G2}
	\task[$\bullet$] Clue on the number. \C[n]{R3}
	\task[$\bullet$] Clue on the colour. \C[c]{B4}
	\task[$\bullet$] Both clues. \C[cn]{M5}
\end{tasks}

We will use letters from A to E to denote players, going clockwise. A string like C \C[c]{B3} \C[cn]{M2} \C{R3} \C{G4} means that player C has those four cards in their hand, and they have been clued on the first two ones. We will use $+$ for the configuration (the played cards).
...
\begin{example}
	...
	\begin{tasks}(3)
		\task[+] \C{R1} \C{Y2} \C{G5} \C{B3} \C{W2} \C{M0}
		\task[A] \C[n]{B4} \C{R3} \C{Y3} \C{W1}
		\task[B] \C[c]{M2} \C{R4} \C{R5} \C{Y2}
		\task[C] \C[c]{M5} \C{R2} \C{Y5} \C{Y1}
		\task[D] \C{B2} \C{G4} \C{Y4} \C{R1}
		\task[E] \C[c]{W4} \C{G2} \C{G3} \C{Y1}
	\end{tasks}
...
\begin{example}
	...
	\begin{tasks}(3)
		\task[+] \C{R0} \C{Y0} \C{G0} \C{B0} \C{W0} \C{M0}
		\task[A] \C{R3} \C{G4} \C{M5} \C{B3}
		\task[B] \C{W2} \C{B1} \C{G2} \C{Y1}
		\task[C] \C{W1} \C{B4} \C{R2} \C{B3}
		\task[D] \C{Y5} \C{Y2} \C{Y1} \C{W5}
		\task[E] \C{G3} \C{W2} \C{M3} \C{G1}
	\end{tasks}
...
\begin{example}[A brilliant first round]
	...
	\begin{tasks}(3)
		\task[+] \C{R0} \C{Y0} \C{G0} \C{B0} \C{W0} \C{M0}
		\task[A] \C{M1} \C{Y1} \C{Y4} \C{W3}
		\task[B] \C{R1} \C{R3} \C{G4} \C{R4}
		\task[C] \C{G1} \C{W2} \C{B2} \C{Y5}
