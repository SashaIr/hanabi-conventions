...
\begin{example}
	...
	In this case is ok to clue a trash card in D's hand, because the clue is adding new important information (D is told that they have a \B{5}, and A is told that their \Z{4} is in fact a \B{4}, see Convention~\ref{prompt}), and also D knows that their other blue card is trash (hence there is no risk of misplays).
\end{example}
...
\begin{convention}[Early saves]
	A \Z{5} or \M{S} clue on the card that is one away from chop is an \textit{early save} on that card, and implies that the card on chop is valuable and should not be discarded.
...
\begin{example}
	...
	\begin{tasks}(3)
		\task[+] \CARD{R4} \CARD{Y3} \CARD{G5} \CARD{B3} \CARD{W5} \CARD{M5}
